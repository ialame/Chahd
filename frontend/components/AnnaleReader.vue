...
<template>
  <div class="space-y-5">
    <div v-if="view.preambleHtml" dir="auto" class="leading-relaxed text-[#6b5f57]" v-html="view.preambleHtml" />

    <section v-for="(sec, i) in view.sections" :key="i" class="space-y-3">
      <!-- Énoncé -->
      <div dir="auto" class="leading-relaxed text-[#6b5f57]" v-html="sec.enonceHtml" />
      ...
      <template v-if="sec.corrigeHtml">
        ...
        <div v-show="open[i]" class="rounded-xl border border-emerald-200 bg-emerald-50/40 p-4">
          ...
          <div dir="auto" class="leading-relaxed text-[#6b5f57]" v-html="sec.corrigeHtml" />
        </div>
      </template>
