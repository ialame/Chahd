  ...
  <div v-if="current" class="space-y-4">
    ...
    <div class="flex items-center justify-between text-xs text-slate-500">
      ...
      <div class="flex gap-1.5">
        <button class="rounded-lg border border-slate-200 px-2 py-1 hover:bg-slate-50" title="Mélanger" @click="reset(true)">
          <i class="fa-solid fa-shuffle" />
        </button>
        <button class="rounded-lg border border-slate-200 px-2 py-1 hover:bg-slate-50" title="Remettre dans l'ordre" @click="reset(false)">
    ...
    >
      ...
      <div class="leading-relaxed text-slate-800" v-html="inlineMath(revealed ? current.reponse : current.question)" />
      <span class="mt-4 text-[11px] text-slate-400">
    ...
    <div class="flex items-center justify-between">
      <button class="btn-ghost" :disabled="idx === 0" @click="prev">← Précédente</button>
      <button class="btn-primary" @click="revealed = !revealed">{{ revealed ? 'Question' : 'Réponse' }}</button>
      <button class="btn-ghost" :disabled="idx >= total - 1" @click="next">Suivante →</button>
    </div>
  </div>

  <p v-else class="py-8 text-center text-sm italic text-slate-400">Aucune fiche pour ce chapitre.</p>
</template>
